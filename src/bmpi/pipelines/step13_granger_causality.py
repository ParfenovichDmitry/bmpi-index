...
Directions tested:
  mentions                 -> btc_logret
  mentions                 -> excess_media_usd
  tone                     -> btc_logret
  tone                     -> excess_media_usd
  excess_media_usd         -> btc_logret
  btc_logret               -> mentions
  bmpi_score               -> btc_logret
...
def build_dataset() -> pd.DataFrame:
    ...
    # If old name missing, use step09 column
    if "excess_media_effect_usd" not in df.columns and "media_effect_used" in df.columns:
        df["excess_media_effect_usd"] = pd.to_numeric(df["media_effect_used"], errors="coerce")
    ...
    for preset in ("balanced", "sensitive", "strong"):
        ...
        m_col = next(
            (c for c in gdf.columns if c in ("mentions", "liczba_wzmianek", "mention_count")),
            None,
        )
        t_col = next(
            (c for c in gdf.columns if c in ("tone", "sredni_tone", "avg_tone", "tone_avg")),
            None,
        )
        ...
        sub = gdf[["date"]].copy()
        if m_col:
            sub[f"mentions_{preset}"] = pd.to_numeric(gdf[m_col], errors="coerce")
        if t_col:
            sub[f"tone_{preset}"] = pd.to_numeric(gdf[t_col], errors="coerce")
    ...
    if feat is not None:
        ...
        if price_col:
            sub["btc_price"] = pd.to_numeric(feat[price_col], errors="coerce")
        if mcap_col:
...
def check_stationarity(df: pd.DataFrame, columns: list) -> pd.DataFrame:
    ...
    print("─" * 72)
    print(f"  {'variable':<30} {'ADF stat':>10} {'ADF p':>8} {'verdict':>18} {'KPSS p':>8}")
    print("  " + "─" * 76)
    ...
    for col in columns:
        ...
        verdict = (
            "✓ stationary" if adf_ok and kpss_ok else
            "✗ non-stat -> diff" if not adf_ok else
            "~ ambiguous"
        )

        print(
            f"  {col:<30} {fmt(adf_stat):>10} {fmt(adf_p):>8} "
...
def run_var_model(df: pd.DataFrame, station_df: pd.DataFrame) -> dict:
    ...
    cols = [
        statcol(v) for v in ("btc_logret", "mentions", "excess_media_effect_usd")
    ...
    try:
        ...
        try:
            ...
            if i_m is not None and i_r is not None:
                ...
                if hl:
                    print(f"\n  Peak: {peak:.6f}  |  Half-life: ~{hl} days")
                else:
                    print(f"\n  Peak: {peak:.6f}")
...
def print_summary(granger_df: pd.DataFrame, var_results: dict) -> None:
    ...
    if len(granger_df) > 0:
        sig_mask = (
            granger_df.get("granger_effect", pd.Series(dtype=bool)).fillna(False)
            if "granger_effect" in granger_df.columns
            else (granger_df.get("p_value", pd.Series(dtype=float)) < 0.05).fillna(False)
        )
        sig = granger_df[sig_mask]
        ...
        for _, row in sig.iterrows():
            ...
            if key in seen:
                continue
            seen.add(key)
            text = f"    ★ {str(key)[:50]:<50} lag={int(row.get('lag', 0))}"
            print(f"║{text:<70}║")
    ...
    if var_results:
        print("║                                                                      ║")
        text = f"  VAR optimal lag: AIC={var_results.get('opt_lag_aic', '?')}  BIC={var_results.get('opt_lag_bic', '?')}"
        print(f"║{text:<70}║")
    ...
    print("║  It means: X has predictive value for Y beyond Y's own history.     ║")
    print("║  This is standard in financial econometrics.                         ║")
    print("║                                                                      ║")
    print("╚" + "═" * (W - 2) + "╝")
